...
def query_univ3_graph(query: str, variables=None,network='mainnet') -> dict:    
    ...
    elif network == 'optimism-perp':
        # perpetual protocol subgraph
        univ3_graph_url = 'https://api.thegraph.com/subgraphs/name/perpetual-protocol/perpetual-v2-optimism'
...
def get_swap_data(contract_address,file_name,DOWNLOAD_DATA = True,network='mainnet', end_time: int = None):       
    """
    ...
    if DOWNLOAD_DATA:
        ...
        if network == 'optimism-perp':
          current_time = int(time.time())
          current_payload = generate_perp_price_payload(contract_address,str(current_time))
          current_id      = query_univ3_graph(current_payload,network=network)['data']['positionChangeds'][0]['timestamp']
        else:
...
def get_perp_swap_data(contract_address,file_name,DOWNLOAD_DATA = True):
  get_swap_data(contract_address,file_name,DOWNLOAD_DATA)
...
def generate_perp_price_payload(addr: str, timestamp: str):
  payload = '''{
    positionChangeds(
      first: 1000
      orderBy: timestamp,
...
def get_price_data_bitquery(token_0_address,token_1_address,date_begin,date_end,api_token,file_name,DOWNLOAD_DATA = True,RATE_LIMIT=False,exchange_to_query='Uniswap'):
    ...
    if DOWNLOAD_DATA:        
        ...
        if request[0]['data']['ethereum']['dexTrades'] != []:
